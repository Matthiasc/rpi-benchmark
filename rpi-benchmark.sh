...
echo -e "System Memory Usage:\e[94m"
free -h
echo -e "\e[93m"

echo -e "Disk Usage:\e[94m"
df -h
echo -e "\e[93m"
...
if [ `which speedtest` ]; then
    SPEEDTEST="$(which speedtest) --simple"
...
sysbench cpu --threads=4 --validate=on --cpu-max-prime=5000 run | grep 'total time:\|min:\|avg:\|max:' | tr -s [:space:]
vcgencmd measure_temp
echo -e "\e[93m"

echo -e "Running THREADS test...\e[94m"
sysbench threads --threads=4 --validate=on --thread-yields=4000 --thread-locks=6 run | grep 'total time:\|min:\|avg:\|max:' | tr -s [:space:]
vcgencmd measure_temp
echo -e "\e[93m"

echo -e "Running MEMORY test...\e[94m"
sysbench memory --threads=4 --validate=on --memory-block-size=1K --memory-total-size=3G --memory-access-mode=seq run | grep 'Operations\|transferred\|total time:\|min:\|avg:\|max:' | tr -s [:space:]
vcgencmd measure_temp
echo -e "\e[93m"
